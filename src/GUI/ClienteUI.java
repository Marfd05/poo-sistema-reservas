...
public class ClienteUI extends javax.swing.JFrame {
    ...
    private javax.swing.JPanel buildReservacionCard(Reservacion reservacion) {
        ...
        javax.swing.JButton cancelarBtn = new javax.swing.JButton();
        ...
        reservacionCard.add(cancelarBtn, new org.netbeans.lib.awtextra.AbsoluteConstraints(485, 65, -1, -1));
        
        cancelarBtn.addActionListener((e) -> {
            
            int index = -1;
            for (int i=0; i<Main.hotel.getReservaciones().leer().size(); i++) {
                Reservacion r = Main.hotel.getReservaciones().leer().get(i);
                if(r.getId().equals(reservacion.getId())) {
                    index = i;
                }
            }
            reservacion.setEstado(EstadoReservacion.CANCELADA);
            Main.hotel.getReservaciones().actualizar(index, reservacion);
            cargarReservaciones();
        });
